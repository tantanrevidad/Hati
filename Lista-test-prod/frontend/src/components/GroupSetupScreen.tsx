  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#F3EFE7] via-[#FCECEE] to-[#EFA8B5]/20 dark:from-[#252B2F] dark:via-[#222123] dark:to-[#2F2427] py-12 px-6 flex flex-col items-center justify-start overflow-y-auto relative">
      ...
      {onBack && (
        <button 
          onClick={onBack}
          className="absolute top-6 left-6 w-10 h-10 bg-white/50 dark:bg-slate-800/50 hover:bg-white dark:hover:bg-slate-800 rounded-full flex items-center justify-center text-[#1B5648] dark:text-slate-300 shadow-sm backdrop-blur-sm transition-all z-20"
        >
      ...
      >
        <div className="text-center mb-8">
          <h2 className="text-4xl font-black font-sans text-[#13463B] dark:text-white mb-2 tracking-tight">Team up!</h2>
          <p className="text-[#316D5F] dark:text-slate-400 text-sm font-medium">Create a new group or join an existing one.</p>
        ...
        <div className="space-y-6 font-sans relative">
          ...
          <div className={`relative z-10 transition-all duration-300 ${joinMethod ? 'opacity-50 scale-95 pointer-events-none' : ''}`}>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl p-6 border-2 border-[#C8DACF] dark:border-slate-800 relative z-10">
              ...
              <div className="space-y-4">
                <input 
                  ...
                  className="w-full px-4 py-3.5 rounded-xl border-2 border-[#C8DACF] dark:border-slate-700 bg-white dark:bg-slate-950 text-[#13463B] dark:text-white placeholder-slate-400 dark:placeholder-slate-600 focus:border-slate-850 dark:focus:border-slate-300 focus:outline-none transition-all font-medium font-sans"
                />
                <button 
                  onClick={handleCreateGroup}
                  className="w-full bg-[#13463B] hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-100 text-white dark:text-slate-900 font-bold py-3.5 rounded-xl transition-all flex items-center justify-center gap-2 border-2 border-slate-900 dark:border-white shadow-md hover:translate-y-[-1px] active:translate-y-[1px]"
                >
          ...
          <div className={`flex items-center justify-center gap-4 text-slate-400 dark:text-slate-500 py-1 transition-opacity ${joinMethod ? 'opacity-50' : ''}`}>
            ...
            <span className="font-roboto font-bold -2xl font-black text-[#316D5F] dark:text-slate-400">Join Existing</span>
            <div className="h-px bg-slate-300 dark:bg-slate-700 flex-1"></div>
          </div>
          ...
          <div className="relative">
            ...
            >
              <button 
                ...
                className="bg-white dark:bg-slate-900 rounded-3xl shadow-xl p-6 border-2 border-[#C8DACF] dark:border-slate-800 hover:border-slate-900 dark:hover:border-white transition-all flex flex-col items-center justify-center gap-3 group relative overflow-hidden hover:translate-y-[-2px] active:translate-y-[1px]"
              >
                <div className="absolute inset-x-0 bottom-0 h-1.5 bg-leaf-yellow/60" />
                <div className="w-12 h-12 rounded-full bg-[#F3EFE7] dark:bg-slate-950 border-2 border-[#C8DACF] dark:border-slate-800 flex items-center justify-center text-[#1B5648] dark:text-slate-300 group-hover:bg-leaf-yellow/30 dark:group-hover:bg-leaf-yellow-dark/20 group-hover:text-[#13463B] dark:group-hover:text-white transition-colors">
                  <QrCode size={22} className="stroke-[2.5]" />
                </div>
                <span className="font-black text-[#1B5648] dark:text-slate-200 text-base tracking-tight">Scan QR</span>
              </button>
              ...
              >
                ...
                <div className="w-12 h-12 rounded-full bg-[#F3EFE7] dark:bg-slate-950 border-2 border-[#C8DACF] dark:border-slate-800 flex items-center justify-center text-[#1B5648] dark:text-slate-300 group-hover:bg-leaf-pink/30 dark:group-hover:bg-leaf-pink-dark/20 group-hover:text-[#13463B] dark:group-hover:text-white transition-colors">
                  <UserPlus size={22} className="stroke-[2.5]" />
                </div>
                <span className="font-black text-[#1B5648] dark:text-slate-200 text-base tracking-tight">Join Link</span>
              </button>
            </div>
                  ...
                  <div className="w-full bg-white dark:bg-slate-900 rounded-3xl shadow-2xl p-2 pl-4 border-2 border-slate-800 dark:border-slate-300 flex items-center gap-2 relative z-20">
                    ...
                    <form onSubmit={handleJoinByLink} className="flex-1 flex items-center">
                      <input 
                        ...
                        className="w-full bg-transparent border-none text-[#13463B] dark:text-white placeholder-slate-400 focus:outline-none font-medium font-sans"
                        autoFocus
                      />
                      <button 
                        type="submit"
                        className="bg-[#13463B] hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-100 text-white dark:text-slate-900 font-bold py-2.5 px-4 rounded-xl transition-all shadow-md ml-2"
                      >
                        Join
                      </button>
                    </form>
                    <button 
                      ...
                      className="text-slate-400 hover:text-[#236450] dark:hover:text-slate-200 p-2"
                    >
                      <X size={20} />
                    </button>
                  </div>
