...
router.post('/settlements', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await db.run(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `, [settlementId, groupId, fromUserId, method, amount, initialStatus, now]);

    const confirmations = [];

    // If QRPH, they are auto-confirmed
    for (const share of creditorShares) {
      const confirmId = uuidv4();
      const confirmedAtVal = method === 'qrph' ? now : null;
      await db.run(`
        INSERT INTO confirmations (id, settlementId, toUserId, amount, confirmedAt)
        VALUES (?, ?, ?, ?, ?)
      `, [confirmId, settlementId, share.toUserId, share.amount, confirmedAtVal]);
    ...
    if (method === 'stellar') {
      try {
        ...
        const sender = await db.get('SELECT walletSecret FROM users WHERE id = ?', [fromUserId]);
        ...
        for (const share of confirmations) {
          ...
          const recipient = await db.get('SELECT walletAddress FROM users WHERE id = ?', [share.toUserId]);
          ...
          await db.run('UPDATE confirmations SET confirmedAt = ? WHERE id = ?', [now, share.id]);
        ...
        await db.run('UPDATE settlements SET status = ?, stellarTxHash = ? WHERE id = ?', [
          'confirmed',
          mainHash,
          settlementId
        ]);
        ...
        await db.run('UPDATE settlements SET status = ? WHERE id = ?', ['failed', settlementId]);
...
router.post('/settlements/:id/confirm', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const now = new Date().toISOString();
    // Update confirmation
    await db.run('UPDATE confirmations SET confirmedAt = ? WHERE id = ?', [now, confirmRow.id]);

    // Check if ALL confirmations are now filled for this settlement
    const allConfirms = await db.all('SELECT confirmedAt FROM confirmations WHERE settlementId = ?', [settlementId]);
    const allDone = allConfirms.every(c => c.confirmedAt !== null);

    if (allDone) {
      await db.run("UPDATE settlements SET status = 'confirmed' WHERE id = ?", [settlementId]);
    }

    // Load full updated settlement details
    const settlement = await db.get('SELECT * FROM settlements WHERE id = ?', [settlementId]);
    settlement.confirmations = await db.all('SELECT * FROM confirmations WHERE settlementId = ?', [settlementId]);
...
router.post('/settlements/qrph/generate', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const recipient = await db.get('SELECT displayName, linkedPaymentMethods, authMethod, phone FROM users WHERE id = ?', [toUserId]);
    if (!recipient) {
