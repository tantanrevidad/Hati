    ...
    <div className="min-h-screen bg-gradient-to-br from-[#F7F5EC] via-[#FCECEE] to-[#EFA8B5]/20 dark:from-[#252B2F] dark:via-[#222123] dark:to-[#2F2427] relative flex flex-col items-center justify-center overflow-hidden">
      ...
      >
        ...
        <motion.div
          ...
          transition={{ duration: 1.2, ease: [0.16, 1, 0.3, 1], delay: 0.1 }}
          className="relative flex flex-col items-center justify-center p-4 mb-4"
        >
          <motion.h1
            initial="hidden"
            animate="visible"
            className="text-7xl md:text-8xl tracking-tight text-slate-900 dark:text-white select-none pointer-events-none pb-2 text-center flex items-center justify-center"
            ...
            <div className="flex items-center justify-center space-x-0.5">
              {[
                ...
                <div key={index} className="relative inline-flex items-center justify-center">
                  {/* Hidden span to enforce natural width */}
                  <span className="opacity-0 invisible px-0.5">
                    {item.char === 'i' ? 'ı' : item.char}
                  </span>
                  
                  {/* Wiping animation (writing simulation) */}
                  <motion.div
                    initial={{ clipPath: 'inset(0 100% 0 0)' }}
                    animate={{ clipPath: 'inset(0 0% 0 0)' }}
                    transition={{ duration: 0.5, ease: "easeInOut", delay: 0.3 + index * 0.15 }}
                  ...
                  >
                    ...
                    {item.greenDot && (
                      <motion.span 
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        transition={{ duration: 0.3, type: "spring", delay: 0.3 + index * 0.15 + 0.3 }}
                        className="absolute top-[0.12em] left-1/2 -translate-x-1/2 w-[0.25em] h-[0.25em] bg-[#10C86E] rounded-full"
                      ></motion.span>
                    )}
                  </motion.div>
                </div>
