...
export default function App() {
  ...
  return (
    <>
      ...
      {currentScreen === 'add_group' && <GroupSetupScreen onBack={() => setCurrentScreen('dashboard')} onNext={(name) => { 
        if (name === 'Joined Group') {
          setGroupName(name);
          setCurrentScreen('dashboard');
        } else {
      ...
      {currentScreen === 'group_qr' && <GroupQRScreen groupName={groupName} onNext={() => setCurrentScreen('dashboard')} />}
      {currentScreen === 'dashboard' && <DashboardScreen expenses={expenses} setExpenses={setExpenses} groups={groups} onCreateGroup={() => setCurrentScreen('add_group')} onAddBillingMethod={() => setCurrentScreen('payment_setup')} onLogout={() => { localStorage.removeItem('userEmail'); setCurrentScreen('login'); }} />}
    </>
  );
}
