...
let newTitleBlock = `<motion.h1
          ...
          >
            ...
            <div className="flex items-end justify-center overflow-hidden px-4 pt-4 pb-2">
              {[
                { char: 'L' },
                ...
                >
                  <div className="relative flex flex-col items-center justify-end h-[1em]">
                    {item.greenDot && (
                      <motion.span 
                        initial={{ scale: 0, opacity: 0 }}
                        animate={{ scale: 1, opacity: 1 }}
                        transition={{ duration: 0.4, type: "spring", delay: 0.1 + index * 0.08 + 0.4 }}
                        className="absolute -top-[0.1em] left-1/2 -translate-x-1/2 w-[0.24em] h-[0.24em] bg-[#10C86E] rounded-full"
                      ></motion.span>
                    )}
                    <span className="leading-none mt-auto">
